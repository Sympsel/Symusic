...
namespace symlog {
    ...
    public:
        ...
        static std::string_view getColorForLevel(const Level& level) {
            switch (level) {
    ...
    public:
        ...
        public:
            ...
            ~Msg() {
                const auto& instance = Log::getInstance();
                if (_head._level >= instance.getLogLevelFilter()) {
                    std::string fullMessage = _formattedHeader + _body;
                    if (ShowSetting::getInstance().isEnabled(LogElem::COLOR)) {
                        fullMessage += Color::RESET;
                    }
            ...
            Msg& operator<<(const QString& value) {
                _body += value.toStdString();
                return *this;
            }
    ...
    private:
        ...
        explicit Log(const OutputTo where = OutputTo::CONSOLE)
            : _where(where), _filter(Level::DEBUG) {
            initAnsiSupport();
            setOutputTo(where);
        }
    ...
    public:
        ...
        const Level& getLogLevelFilter() const {
            return _filter;
        }

        void setLogLevelFilter(const Level minLevel) {
            _filter = minLevel;
        }
        ...
        Msg operator()(const Level level,
                       const std::string& filename,
