module symusic.component.list_item;
...
ListItem::ListItem(PlayContext songCtx)
    ...
    connect(_likeButton, &QPushButton::clicked, this, [this]() {
        ...
        if (const auto& song = _songCtx.song; !song->isLiked()) {
            ...
            logDebug() << "添加到喜欢列表: " << song->getName();
        } else {
            songManager.remove(likedList, song->getId());
            logDebug() << "从喜欢列表移除: " << song->getName();
        }
        updateIconStatus();
...
void ListItem::mousePressEvent(QMouseEvent* event) {
    if (event->button() == Qt::LeftButton) {
        ...
        logDebug() << "mousePressEvent, pending=" << _pendingSingleClick;
        if (_pendingSingleClick) {
            _clickTimer->stop();
            _pendingSingleClick = false;
            return;
...
void ListItem::mouseDoubleClickEvent(QMouseEvent* event) {
    if (event->button() == Qt::LeftButton) {
        logDebug() << "mouseDoubleClickEvent 触发";
        _clickTimer->stop();
        _pendingSingleClick = false;
        _skipNextPress = true;
