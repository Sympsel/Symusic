...
export module symusic.entity.play_manager;
export import symusic.common; 
export import symusic.entity.song_manager;
export import symusic.entity.status_manager;


/**
 * @brief 歌曲上下文，记录触发播放逻辑时的歌曲和所在列表
 */
export struct PlayContext {
    SongPtr song;
    SongList* list;

    PlayContext(SongPtr song, SongList& listRef)
...
export class PlayManager : public QObject {
    Q_OBJECT
...
private:
    explicit PlayManager()
        ...
        connect(_player, &QMediaPlayer::errorOccurred, this, [this](
                const QMediaPlayer::Error error, const QString& errorString) {
                    logError() << "播放器错误:" << errorString << "错误码:" << static_cast<int>(error);
    ...
    void initAudioOutput() {
        ...
        if (const QAudioDevice defaultDevice = QMediaDevices::defaultAudioOutput(); defaultDevice.isNull()) {
            logWarn() << "默认音频输出设备为空，使用默认构造函数";
            _audioOutput = new QAudioOutput(this);
        } else {
            logInfo() << "初始化音频输出设备:" << defaultDevice.description();
    ...
    void checkAndRefreshAudioOutput() {
        ...
        if (const QString currentDeviceName = currentDevice.description(); _lastDeviceName != currentDeviceName) {
            logInfo() << "检测到音频设备变化:" << _lastDeviceName << "->" << currentDeviceName;

            if (_player->playbackState() == QMediaPlayer::PlayingState) {
                logInfo() << "音频设备切换，暂停播放并重新初始化";
                _player->pause();
...
public:
    ...
    void play(const PlayContext& songCtx) {
        initAudioOutput();
        ...
        logDebug() << std::format("当前播放歌曲 {}，所在歌单 {}",
